...
\begin{table}[t]
  \centering\small
  \caption{\textbf{Main result} (deployment-matched replay, 25 held-out AMI stretches, energy gate on; latency-vs-false-fire view in Figure~\ref{fig:tradeoff}). v9 dominates every predecessor without either crutch: its \emph{raw} false-fire rate beats v5's \emph{confirmed} rate, and its WER needs no force-flush. The confirm ablation: v5's horizon cancels 241 phrase-boundary candidates; v9's cancels 3. $^{\dagger}$v8's WER mean is a long-monologue repetition tail, fixed only by force-flush. \pending{E13: fresh 50-stretch confirmation set row}}
  \label{tab:main}
  \begin{tabular}{lccccc}
    \toprule
    ...
    v9 + gate (fresh stretches) & \multicolumn{5}{c}{\pending{E13}} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Single-speaker, single-channel English; the mixed-channel meeting shape is future work. Semantic completeness labels come from a heuristic transcript classifier (fillers, connectives, short-utterance rules); a learned completeness judge is the obvious next lever on the residual 3\% recall gap. Distractor hallucination (3.7\%) exceeds our 3\% target. Quick resumes are always segmented (resume rate 1.0); merging them costs $+0.5$\,s via the confirm knob or requires post-hoc fusion. Concurrency numbers are contention-caveated lower bounds from a shared GPU. The benchmark's 25 stretches (96 turn-final boundaries) were used during development; the fresh-stretch confirmation run is \pending{E13}. Offline WER regression is \pending{E12}.
...
\paragraph{Fresh-set confirmation.} \pending{E13: v9+gate on the seed-1, 50-stretch set (running); to be read against the seed-1 timeout rows above}
