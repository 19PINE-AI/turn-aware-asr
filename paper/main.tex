...
Single-speaker, single-channel English; the mixed-channel meeting shape is future work. The completeness heuristic captures \emph{incompleteness} cues --- trailing fillers and connectives, mid-word cuts, short fragments --- but not \emph{continuation-expecting} structure: a half-dictated phone number reads as complete, so the model will fire in long inter-group pauses unless the confirm horizon absorbs them, and the benchmark contains no dictation material to measure this on. Closing that gap --- a dictation probe set (numbers, emails, addresses) and a continuation-aware schema --- is the highest-priority follow-up, since it is exactly the scenario (\S\ref{sec:motivation}, Example 1) where semantic endpointing should shine; a learned completeness judge is the corresponding lever on the residual recall gap. Each supervision recipe in Table~\ref{tab:composition} was trained once; multi-seed replication of the oscillation-vs-monotone contrast, and a component ablation of the causal recipe, are the immediate next experiments. Spelled-out names and emails are motivated in \S\ref{sec:motivation} but not yet probed; the biasing evidence covers naturally spoken entities. Distractor hallucination (3.7\%) exceeds our 3\% target. Quick resumes are always segmented (resume rate 1.0); merging them costs $+0.5$\,s via the confirm knob or requires post-hoc fusion. Concurrency numbers are contention-caveated lower bounds from a shared GPU. The development benchmark was reused during development; the fresh-set numbers (recall 0.924, overlapping Wilson intervals, \S\ref{sec:results}) are the honest operating estimate. The endpoint fine-tune costs offline WER (\S\ref{sec:capability}): mixing plain ASR data into the fine-tune is the standard, unexercised mitigation.
