...
\begin{enumerate}\itemsep2pt
  ...
  \item \textbf{A causal label recipe} (\S\ref{sec:v9}) whose minimal-pair construction (Figure~\ref{fig:minimalpair}) turns the former contradiction into the intended discriminative feature, yielding 0.97 recall at 0.39\,s P50 and 0.3 false fires/min with no inference-time crutches.
  \item \textbf{Numbers for a capability with no public numbers} (\S\ref{sec:rest}): the context prefix lifts hotword recall by $+28.9$\,pp on Earnings-22, flat across session age (Figure~\ref{fig:biasing}); offline WER is \pending{E12}.
...
\paragraph{Offline ASR regression.} Table~\ref{tab:wer}: the endpoint fine-tune costs $+1.1$\,pp WER on LibriSpeech test-clean and $+2.7$\,pp on test-other when the merged checkpoint is decoded \emph{as-is} in offline mode. Two observations before interpreting this as capability loss. First, the model fires its marker on 4.7--5.9\% of offline clips --- and a fire ends the decode, truncating whatever transcript remains; unlike deployment streams, offline clips are exactly the distribution where firing is ambiguous (many LibriSpeech clips do carry a ${\geq}0.3$\,s silence tail, where the v9 rule \emph{says} fire). Second, the fix for offline use is configuration, not retraining: ban the two marker tokens at decode time. \pending{E12-suppressed: WER with markers banned --- isolates truncation from genuine drift; running}. The deployment-relevant streaming WER is unaffected (Table~\ref{tab:main}).
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    + markers banned at decode (offline mode) & \multicolumn{4}{c}{\pending{E12-suppressed}} \\
    \bottomrule
  \end{tabular}
\end{table}
