...
\section{Closing the loop on \S\ref{sec:motivation}: dictation and spelled entities}
\label{sec:dictation}

The two motivating scenarios deserve measurement, not anecdote, so we built probes for both --- and used them to drive one further data-only iteration. The \textbf{dictation probe} replays 50 ten-digit phone numbers (3-3-4 groups from held-out Free Spoken Digit Dataset speakers, 0.6--1.2\,s inter-group pauses) through the exact streaming stack of \S\ref{sec:eval}; the \textbf{spelled-entity probe} decodes 80 synthesized utterances that spell uncommon names and email addresses, with and without a user-profile context prefix, plus a wrong-user \emph{distractor} profile. Probe speakers, voices, and identities are all disjoint from training.

\paragraph{The flagship model fails the dictation probe --- as predicted.} Its completeness heuristic reads a digit group as a finished thought: 4.8 premature fires per dictated number, every sequence interrupted --- \emph{worse than a 0.5\,s timeout} (2.0). Spelled emails are unusable: 0\% exact without context, 40\% with the profile. The \S\ref{sec:motivation} examples are real failure modes of our own system as shipped in \S\ref{sec:results}.

\paragraph{Five schemas close the gap.} We extend the recipe with dictation and context schemas, built under the same causal discipline: partial phone numbers (any 1--9 digit prefix) followed by a pause are labeled \emph{hold} --- the pattern predicts continuation; full ten-digit numbers with the same internal pauses fire after the usual ${\geq}0.3$\,s tail (and, without a tail, do not); spelled names and emails are targeted in normalized written form (\texttt{anna.kowalski@pine.ai}), half of them with a user profile in the context slot. Retraining the same LoRA on the extended pool (${\sim}14.6$k examples, hours on one GPU) yields Table~\ref{tab:dictation}: premature fires drop $4.8 \to 0.36$ per number --- now better than \emph{every} timeout setting --- digit accuracy reaches 0.996, spelled names hit 100\% exact even without context, and emails go from unusable to 92.5\% exact with the profile prefix.

\begin{table}[t]
  \centering\small
  \caption{\textbf{Dictation and spelled-entity probes.} Left: premature end-of-turn fires per dictated 10-digit number and final-boundary behavior (window $+1.75$\,s). Right: exact-match rates on spelled names and emails without/with the user-profile context, and the wrong-profile intrusion rate. The dictation-extended model beats every timeout on premature fires while keeping sub-second final latency; its residual misses are $\sim$6 shredded-by-premature and $\sim$5 fires landing $\sim$2\,s after speech end, just outside the window.}
  \label{tab:dictation}
  \setlength{\tabcolsep}{3pt}
  \footnotesize
  \begin{tabular}{lccc|ccc}
    \toprule
    & Premat./seq\,$\downarrow$ & Final rec.\,$\uparrow$ & Digit acc\,$\uparrow$ & Name ${-}/{+}$ctx & Email ${-}/{+}$ctx & Intrus.\,$\downarrow$ \\
    \midrule
    flagship (\S\ref{sec:results}) & 4.80 & 0.76 & 0.910 & 0.30\,/\,0.83 & 0.00\,/\,0.40 & 1.3\% \\
    timeout $X{=}0.5$\,s & 2.00 & 1.00 & --- & --- & --- & --- \\
    timeout $X{=}1.0$\,s & 0.74 & 1.00 & --- & --- & --- & --- \\
    \textbf{+ dictation schemas} & \textbf{0.36} & 0.78 & \textbf{0.996} & \textbf{1.00\,/\,1.00} & 0.03\,/\,\textbf{0.93} & 11.3\% \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{The cost surface, reported as-is.} The extension is not free at fixed adapter capacity. On the conversational benchmark of \S\ref{sec:results}, the extended model's recall is 0.917 vs the flagship's 0.969, with 0.97 vs 0.30 false fires per speech-minute; and teaching the model to \emph{use} the profile makes it more gullible --- wrong-profile intrusion rises from 1.3\% to 11.3\%, and a longer-trained checkpoint that reaches 0.12 premature fires and 0.88 final recall pays 40\% intrusion, which is why we reject it. This is a monotone breadth-vs-precision trade-off across checkpoints of a single healthy run --- not the oscillation pathology of \S\ref{sec:phantom}; the causal-label fingerprints stay clean throughout. We release both models: the flagship for open-domain conversation, the extension for form-filling and dictation-heavy deployments, with the trade-off quantified so the choice is the deployer's. Distractor-ratio training and plain-ASR replay data are the known, still-unexercised levers on the intrusion and recall costs.
