#!/bin/bash
# v10 endpoint fine-tune with OOM-resilient restart (shared-GPU co-tenant).
# Same recipe as the causal (v9) run; only the data and holdout size differ.
set -uo pipefail
cd "$(dirname "$0")/.."

CKPT_DIR=checkpoints/semantic_endpoint_v10_es
mkdir -p "$CKPT_DIR"

for attempt in $(seq 1 30); do
    echo "=== attempt $attempt $(date -Is) ==="
    .venv/bin/python -m src.train_semantic_endpoint \
        --data data/semantic_endpoint_v10/data.pt \
